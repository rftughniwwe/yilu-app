	...
	<view class="class_list">
		...
		<view class="class_box" v-if="isClass" @tap="showOption(0)">
			...
			<scroll-view scroll-y="true" class="class_items">
				...
				<view v-for="(item, index) in classList" :key="index" :class="'class_item ' + (map.categoryId1 === item.id ? 'c_blue' : '')"
				 @tap.stop="selectCate(item)">{{item.categoryName}}</view>
			</scroll-view>
			<!-- 二级分类 -->
			<scroll-view scroll-y="true" class="class_items" v-if="twoClassList.length">
				...
				<view v-for="(item, index) in twoClassList" :key="index" :class="'class_item ' + (map.categoryId2 === item.id ? 'c_blue' : '')"
				 @tap.stop="selectCate(item)">{{item.categoryName}}</view>
			</scroll-view>
			<!-- 三级分类 -->
			<scroll-view scroll-y="true" class="class_items" v-if="threeList.length">
				...
				<view v-for="(item, index) in threeList" :key="index" :class="three === item.categoryNo ? 'class_item c_blue' : 'class_item'"
				 @tap.stop="changeThree(item)">{{item.categoryName}}</view>
			</scroll-view>
		</view>
		<!-- 是否免费 -->
		<scroll-view scroll-y="true" class="free_box" v-if="ctrl.optionVisible == 2" @tap="showOption(0)">
			...
			<view v-for="(item, index) in opts.free" :key="index" :class="'free_item ' + (item.val == map.isFree? 'c_blue': (item.val == 2 && map.isVipFree)? 'c_blue' : '' )"
			 @tap="selectOption({name: item.key, free:item.val })">{{item.key}}</view>
		</scroll-view>
		<!-- 课程类型 -->
		<scroll-view scroll-y="true" class="free_box" v-if="ctrl.optionVisible == 1" @tap="showOption(0)">
			<view v-for="(item, index) in opts.courseType" :key="index" :class="'free_item ' + (item.val == map.courseCategory? 'c_blue': '')"
			 @tap="selectOption({name: item.key, cate:item.val })">{{item.key}}</view>
		</scroll-view>
	</view>
</template>
		...
		methods: {
			...
			getClassList() {
				apis.categoryList({
					...
					this.classList = res.courseCategoryList
				});
			},
			...
			selectCate(cate) {
				...
				if (cate.floor === 1) {
					...
					map.categoryId2 = '';
					map.categoryId3 = '';
					this.className = cate.categoryName || '全部分类'
					this.twoClassList = cate.twoList || []
					this.map = map
